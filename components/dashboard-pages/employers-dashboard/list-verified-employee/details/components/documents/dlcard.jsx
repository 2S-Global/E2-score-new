...
const DlDetails = ({ user }) => {
    ...
    const permanentAddress = dl.user_address?.find((addr) => addr.type === "Permanent");
    ...
    return (
        ...
        <div className="col-md-6 mb-2"><strong>Father/Husband Name:</strong> {user?.dl_response?.result?.father_or_husband || "N/A"}</div>
            ...
            <div className="col-md-6 mb-2"><strong>Blood Group:</strong> {user?.dl_response?.result?.user_blood_group || "N/A"}</div>
            <div className="col-md-6 mb-2"><strong>Expiry Date:</strong> {user?.dl_response?.result?.expiry_date || "N/A"}</div>
            <div className="col-md-6 mb-2"><strong>Issued Date:</strong> {user?.dl_response?.result?.issued_date || "N/A"}</div>
      ...
      <div className="row mt-3">
        {["Permanent", "Present"].map((type) => {
          const address = type === "Permanent" ? permanentAddress : presentAddress;
          return (
  ...
  <div className="col-md-6 mb-2">
    ...
    {dl.transport_validity && dl.transport_validity.from !== "NA" 
      ...
      <div className="row mt-3">
        ...
        <div className="col-md-6 mb-2"><strong>Endorsement Date:</strong> {dl.endorse_date !== "NA" ? dl.endorse_date : "N/A"}</div>
      </div>
          ...
          <ul className="list-group">
            {dl.vehicle_category_details.map((vehicle, index) => (
              <li key={index} className="list-group-item">
                <strong>{vehicle.cov}:</strong> Valid from {vehicle.issueDate} to {vehicle.expiryDate}
              </li>
            ))}
          </ul>
        ) : (
