...
const Testimonialtable = ({ setRefresh, refresh }) => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.post(
        `${apiurl}/api/testimonials/delete-testimonial`,
        { id: id},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const filtered = useMemo(() => {
    return testimonials.filter((Testimonial) => {
      ...
      return (
        Testimonial.customer_name?.toLowerCase().includes(search) ||
        Testimonial.customer_designation?.toLowerCase().includes(search)
      );
    });
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: (row) => row.customer_designation,
      sortable: true,
      width: "",
      center: true,
    },
 ...
 {
      name: "Image",
      selector: (row) => row.customer_image,
      sortable: true,
      width: "",
      center: true,
      cell: (row) => (
        ...
        {row?.customer_image && 
         <Image width='70' height='70' src={row?.customer_image} alt="Testimonial" />
        
        }
        {!row?.customer_image && "No Image"}
        </div>
      ),
    },
